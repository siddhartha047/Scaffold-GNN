...
parse_common_args "$@"
export BASELINE_METHOD_NAME="adaglt"
setup_common_env
require_tunedgnn_preset_contract

ADAGLT_DIR="${SUPPORT_GRAPH_ROOT}/RelatedMethods/AdaGLT-main/Ada_gcn"
DEVICE_ARG="$(cuda_device_arg)"
MASK_EPOCH="${ADAGLT_MASK_EPOCHS:-400}"
RETAIN_EPOCH=300
...
TARGET_SPARSITY="$(awk -v keep="${KEPT_RATIO}" \
    'BEGIN { printf "%.8f", (1.0 - keep) * 100.0 }')"
SEED_ARGS=()
...
[[ -n "${TUNEDGNN_RESIDUAL_CONNECTIONS}" ]] && TUNED_ARGS+=(--use_res_value "${TUNEDGNN_RESIDUAL_CONNECTIONS}")
[[ -n "${TUNEDGNN_LAYER_NORM}" ]] && TUNED_ARGS+=(--use_ln "${TUNEDGNN_LAYER_NORM}")
[[ -n "${TUNEDGNN_BATCH_NORM}" ]] && TUNED_ARGS+=(--use_bn_value "${TUNEDGNN_BATCH_NORM}")
...
case "${DATASET_KEY}" in
    coauthor-physics|questions|roman-empire|reddit|reddit2|ogbn-products|ogb-products|products|ogbn-arxiv|ogb-arxiv|arxiv|ogbn-proteins|ogb-proteins|proteins|pokec)
        ...
        exec "${PYTHON_BIN}" "${SCRIPTS_ROOT}/methods/scalable_sparse_node.py" \
            ...
            --device "${DEVICE}" \
            --epochs "${EPOCHS_OVERRIDE:-$RETAIN_EPOCH}" \
            --runs "${RUNS}" \
            --seed "${SEED}" \
...
if [[ -n "${EPOCHS_OVERRIDE}" ]]; then
    # Mask discovery is method overhead. The retained sparse GCN receives the
    # complete tunedGNN training budget used by the full-graph comparator.
    RETAIN_EPOCH="${EPOCHS_OVERRIDE}"
fi
printf '[RuntimeBudget] mask_search_epochs=%s fixed_ticket_epochs=%s\n' \
    "${MASK_EPOCH}" "${RETAIN_EPOCH}"

if [[ "${DATASET_KEY}" == "cora" ]]; then
    # AdaGLT's documented Cora GCN command uses 0.005. Its generic 0.1
    # default prunes Cora too quickly and captures a collapsed target mask.
    METHOD_ARGS+=(--coef 0.005)
fi
...
exec "${PYTHON_BIN}" main.py \
    ...
    --device "${DEVICE_ARG}" \
    --spar_adj \
    --target_adj_spar "${TARGET_SPARSITY}" \
    --total_epoch "${MASK_EPOCH}" \
    --retain_epoch "${RETAIN_EPOCH}" \
    --runs "${RUNS}" \
    "${SEED_ARGS[@]}" \
    "${METHOD_ARGS[@]}" \
    "${TUNED_ARGS[@]}" \
    --save "${CACHE_ROOT}/adaglt" \
    "${EXTRA_ARGS[@]}"
