...
from BenchmarkDataset import DEFAULT_SPLIT_PROTOCOL, split_fingerprint
...
def load_data(dataset, data_dir=None):
    ...
    # Benchmark also uses OGB's official fixed split for ogbn-proteins.  Keep
    ...
    print(
        "[BenchmarkDataset] "
        f"dataset={dataset} "
        f"split_protocol={os.environ.get('SCAFFOLD_SPLIT_PROTOCOL', DEFAULT_SPLIT_PROTOCOL)} "
        "split_index=0 "
