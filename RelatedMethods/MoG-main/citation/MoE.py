...
class MoE(nn.Module):
    ...
    def forward(self, x, edge_index, temp,shape, edge_attr=None, training=None):
        ...
        k_per_node_all = torch.sum(node_gates * torch.unsqueeze(self.k_list,0),dim=1) # [num_nodes,num_experts]*[1,num_experts]-->[num_nodes,num_experts]-sum->[num_nodes]
        k_per_node = k_per_node_all[node_idx]
