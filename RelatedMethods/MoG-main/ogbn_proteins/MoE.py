...
class MoE(nn.Module):
    ...
    def __init__(self, input_size,hidden_size, num_experts,nlayers,activation,k_list,expert_select ,noisy_gating=True, coef=1e-2,lam=1):
        ...
        self.k_list = k_list
        # instantiate experts
        self.experts = nn.ModuleList([SpLearner(nlayers=nlayers, in_dim=input_size*2+8, hidden=hidden_size, activation=activation, k=k) for k in k_list])
    ...
    def forward(self, x, edge_index, temp, edge_attr=None, training=None):
        ...
        for i in range(self.num_experts):
            ...
            expert_outputs.append(expert_i_output)  # list[Tensor]
        ...
        # gated_output = min_max_normalization(gated_output)
        ...
        # Partition/neighbor batches need not expose every local node as an
        # edge source. Index the gate-derived budget by the actual sources.
        k_edges_per_node = (
        ...
        edge_start_indices = torch.cat((torch.tensor([0],device=edge_index.device), torch.cumsum(num_edges_per_node[:-1], dim=0))) 
